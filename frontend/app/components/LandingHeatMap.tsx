...
export default function LandingHeatMap() {
  ...
  return <div className={`${styles.landingHeatMap} ${active ? styles.landingHeatMapActive : ''}`} style={{ '--heat-x': `${point.x}%`, '--heat-y': `${point.y}%` } as CSSProperties} aria-hidden="true"><div className={styles.heatField} /><div className={styles.heatGrid} /><div className={`${styles.heatPulse} ${styles.heatPulseA}`} /><div className={`${styles.heatPulse} ${styles.heatPulseB}`} /><div className={styles.heatCore} /><div className={styles.heatCode}>{`0101  // hover.signal\n> archive temperature: rising\n> map.pointer(${Math.round(point.x)}, ${Math.round(point.y)})`}</div></div>;
}
